...
def clear_staged_pane_input_if_needed(pane_id: str, *, timeout: int = 8) -> tuple[bool, str]:
    ...
    last_detail = ""
    for args in (
        [herdr_bin(), "pane", "send-keys", pane_id, "ctrl+u"],
        [herdr_bin(), "pane", "send-text", pane_id, "\x15"],
    ):
        proc = run_cmd(args, timeout=timeout)
        if proc.returncode != 0:
            last_detail = sanitize_text(proc.stderr or proc.stdout, 800)
            continue
        for delay in (0.1, 0.2, 0.35):
            time.sleep(delay)
            if not pane_input_looks_staged(pane_id):
                return True, ""
    suffix = f" Last clear error: {last_detail}" if last_detail else ""
    return (
        False,
        "Could not clear existing staged pane input; refusing to append Telegram text. "
        "The pane may be busy or waiting with text already in its prompt. "
        "Submit or clear the staged input in Herdr, then retry from Telegram."
        + suffix,
    )
...
def event_once() -> dict[str, Any]:
    ...
    if not pane or not pane.get("agent"):
        ...
        save_state(state)
        return {"ok": True, "changed": grouping_reset, "pane_id": pane_id, "message": "pane not found or not an agent"}
