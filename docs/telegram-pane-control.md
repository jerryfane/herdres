...
Telegram reach a pane (including when the agent is busy). It consolidates four
changes:

| PR | Change |
|----|--------|
| #1 | Per-agent Telegram topics (`HERDR_TELEGRAM_TOPICS_PER_AGENT`) |
| #2 | Treat input queued behind a busy agent as *queued*, not a send failure |
| #3 | `/send!` — interrupt a busy agent and deliver now |
| #4 | Don't treat Codex's dimmed placeholder as staged input |
